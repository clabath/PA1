...
public class Node { // the node represents an endpoint
	...
	private int maxval; // TODO maximum value obtained by s(leftmost, i) for leftmost <= i <= rightmost
						// TALK TO ME ABOUT THIS I HAVE CONCERNS
	private Endpoint emax; // the key (endpoint) that maximizes s(leftmost, i) for leftmost <= i <=
							// rightmost
	private int ID;
	...
	public Endpoint getEmax() {
		if (getKey() < 0) {
			...
			emax = left.emax;
		} else if (Math.max(left.maxval,
				Math.max(left.maxval + p, left.maxval + p + right.maxval)) == (left.maxval + p)) {
			emax = key;
		} else {
			emax = right.emax;
		}
	...
	/**
	 * Creates a new nil node with p = -1, key = -1, val = 0, maxval = 0 and emax =
	 * key of -1
	 * 
	 * @return
	 */
	public static Node createNILNode() {
		Node nil = new Node(-1, -1, 0); // key, p, ID
		nil.setColor(1);
		nil.getVal();
		nil.getMaxVal();
		nil.getEmax();
